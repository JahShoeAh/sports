...
const completeVenueMapping = () => {
  return new Promise((resolve, reject) => {
    console.log('Starting complete venue mapping...');
          ...
          .on('data', (row) => {
            ...
            if (arena && arena.trim()) {
              ...
              if (homeTeamId && visitorTeamId) {
                // Create a game key based on date and team IDs
                const gameKey = `${date}_${visitorTeamId}_${homeTeamId}`;
          ...
          .on('end', () => {
            ...
            db.all(`SELECT id, game_date, home_team_id, away_team_id FROM games`, (err, games) => {
              ...
              setTimeout(() => {
                ...
                console.log(`\n📊 Complete Venue Mapping Summary:`);
                console.log(`- Updated ${updatedGames} games`);
                console.log(`- Not found ${notFoundGames} games`);
                console.log('🎉 Complete venue mapping completed successfully!');
                resolve();
              }, 3000);
            });
          })
...
if (require.main === module) {
  completeVenueMapping()
    ...
    .catch((error) => {
      console.error('Complete venue mapping failed:', error);
      db.close();
      process.exit(1);
    });
}

module.exports = { completeVenueMapping };
