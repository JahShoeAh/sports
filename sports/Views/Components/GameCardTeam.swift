...
struct GameCardTeam: View {
    let game: Game
    let viewingTeam: Team
    ...
    private var isHomeGame: Bool {
        game.homeTeam.id == viewingTeam.id
    }
        ...
        NavigationLink(destination: GameMenuLoaderView(gameId: game.id)) {
            VStack(alignment: .leading, spacing: 0) {
                ...
                HStack(alignment: .center, spacing: 16) {
                    ...
                    // Right side - Opposing team abbreviation
                    VStack(alignment: .trailing, spacing: 4) {
                        Text(opposingTeam.abbreviation)
                            .font(.title2)
                            .fontWeight(.bold)
                            .foregroundColor(.primary)
                    }
